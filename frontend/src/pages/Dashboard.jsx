                    ...
                    <AnimatePresence>{showFilters && (<motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} className="lg:w-80 flex-shrink-0 overflow-hidden"><div className="h-full overflow-y-auto space-y-3 pr-1">
                        <div className="bg-gradient-to-br from-[#163A6B] to-[#102A4C] rounded-xl p-3 shadow-lg border border-[#1E90FF]/20">
                            ...
                            <div className="space-y-2">
                                <div><label className="text-[10px] text-[#AFC8E6]">Marca</label><select onChange={(e) => updateFilter('brands', e.target.value ? [e.target.value] : [])} className="w-full px-2 py-1.5 bg-[#0B1E3A]/80 border border-[#1E90FF]/30 rounded-lg text-xs"><option value="">Todas</option>{BRANDS.map(b => <option key={b} value={b}>{b}</option>)}</select></div>
                                <div><label className="text-[10px] text-[#AFC8E6]">Rin</label><select value={selectedRim} onChange={(e) => setSelectedRim(e.target.value)} className="w-full px-2 py-1.5 bg-[#0B1E3A]/80 border border-[#1E90FF]/30 rounded-lg text-xs"><option value="">Seleccionar rin</option>{Object.keys(TIRE_SIZES_BY_RIM).map(r => <option key={r} value={r}>{r}</option>)}</select></div>
                                {selectedRim && (<div><label className="text-[10px] text-[#AFC8E6]">Medida</label><select onChange={(e) => updateFilter('sizes', e.target.value ? [e.target.value] : [])} className="w-full px-2 py-1.5 bg-[#0B1E3A]/80 border border-[#1E90FF]/30 rounded-lg text-xs"><option value="">Todas</option>{TIRE_SIZES_BY_RIM[selectedRim].map(s => <option key={s} value={s}>{s}</option>)}</select></div>)}
                                <div><label className="text-[10px] text-[#AFC8E6]">Tipo</label><select onChange={(e) => updateFilter('types', e.target.value ? [e.target.value] : [])} className="w-full px-2 py-1.5 bg-[#0B1E3A]/80 border border-[#1E90FF]/30 rounded-lg text-xs"><option value="">Todos</option><option>Verano</option><option>Invierno</option><option>Todo tiempo</option></select></div>
                                <div className="grid grid-cols-2 gap-2"><input type="number" placeholder="Precio min" value={filters.priceMin} onChange={(e) => updateFilter('priceMin', e.target.value)} className="px-2 py-1.5 bg-[#0B1E3A]/80 border border-[#1E90FF]/30 rounded-lg text-xs" /><input type="number" placeholder="Precio max" value={filters.priceMax} onChange={(e) => updateFilter('priceMax', e.target.value)} className="px-2 py-1.5 bg-[#0B1E3A]/80 border border-[#1E90FF]/30 rounded-lg text-xs" /></div>
                                <div className="grid grid-cols-2 gap-2"><input type="number" placeholder="Stock min" value={filters.stockMin} onChange={(e) => updateFilter('stockMin', e.target.value)} className="px-2 py-1.5 bg-[#0B1E3A]/80 border border-[#1E90FF]/30 rounded-lg text-xs" /><input type="number" placeholder="Stock max" value={filters.stockMax} onChange={(e) => updateFilter('stockMax', e.target.value)} className="px-2 py-1.5 bg-[#0B1E3A]/80 border border-[#1E90FF]/30 rounded-lg text-xs" /></div>
                                <label className="flex items-center gap-2"><input type="checkbox" checked={filters.onlyOfficial} onChange={(e) => updateFilter('onlyOfficial', e.target.checked)} className="w-3 h-3" /><span className="text-[10px] text-[#AFC8E6]">Solo tiendas oficiales</span></label>
